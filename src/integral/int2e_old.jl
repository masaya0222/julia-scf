...
export get_v2e,get_v2e_, get_v2e_single, get_v2e_single2, cont_V2
...
function cont_V2_(basis_a::orb_detail, basis_b::orb_detail, basis_c::orb_detail, basis_d::orb_detail)
    w_fact = [1,1,3,15,105]
    am1 = basis_a.orb_l; am2 = basis_b.orb_l
    am3 = basis_c.orb_l; am4 = basis_d.orb_l
    am = am1 + am2 + am3 + am4
...
function V2e_lm_(basis_a::orb_detail, basis_b::orb_detail, basis_c::orb_detail, basis_d::orb_detail, C_a::Array{Float64,4}, C_b::Array{Float64,4}, C_c::Array{Float64,4}, C_d::Array{Float64,4})
    V2e_abcd = cont_V2_(basis_a, basis_b, basis_c, basis_d)
...
function get_v2e_(mol::Molecule)
    basis = mol.basis_
    num = 0
    V2e = zeros(Float64, (mol.basis_num, mol.basis_num, mol.basis_num, mol.basis_num))
    basis_len = length(basis)
    ...
    Threads.@threads for ind = 1:num
        results[ind] = V2e_lm_(basis[Tasks[ind][1]],basis[Tasks[ind][2]],basis[Tasks[ind][3]],basis[Tasks[ind][4]],mol.rotate_coef[Tasks[ind][5]+1],mol.rotate_coef[Tasks[ind][6]+1],mol.rotate_coef[Tasks[ind][7]+1],mol.rotate_coef[Tasks[ind][8]+1])
    end
            ...
            for k = 1:basis_len
                ...
                for l = 1:basis_len
                    ...
                    if !check[i,j,k,l] && I1 >= J1 && I2 >= J2 && I2+J2 >= I1+J1
                        check[i,j,k,l] = check[i,j,l,k] = check[j,i,k,l] = check[j,i,l,k] = true
                        check[k,l,i,j] = check[k,l,j,i] = check[l,k,i,j] = check[l,k,j,i] = true
                        V2elm = V2e_lm_(basis[i],basis[j],basis[k],basis[l],mol.rotate_coef[I1+1],mol.rotate_coef[J1+1],mol.rotate_coef[I2+1],mol.rotate_coef[J2+1])
...
function get_v2e_single2(mol::Molecule)
    basis = mol.basis
    V2e = zeros(Float64, (mol.basis_num, mol.basis_num, mol.basis_num, mol.basis_num))
    basis_len = length(basis)
    check = zeros(Bool, (basis_len, basis_len, basis_len, basis_len))
